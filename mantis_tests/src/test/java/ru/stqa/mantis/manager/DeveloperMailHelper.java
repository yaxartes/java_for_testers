...
public class DeveloperMailHelper extends HelperBase{
    ...
    public void deleteUser(DeveloperMailUser mailUser) {
        Request request = new Request.Builder()
                .url(String.format("https://www.developermail.com/api/v1/mailbox/%s", mailUser.name()))
                .header("X-MailboxToken", mailUser.token())
                .delete()
                .build();
    ...
    public String receive(DeveloperMailUser mailUser, Duration duration) {
            ...
            try {
                ...
                if(response1.result().size() > 0) {
                    var text2 = get(String.format("https://www.developermail.com/api/v1/mailbox/%s/messages/%s",
                                    mailUser.name(), response1.result().get(0)),
                            mailUser.token());
